...
public class Repairkit : MonoBehaviour
{
    ...
    */
    ...
    {
        if (other.gameObject.name == "player1" && GameManager.Instance.p1.health < 100)
        {
            GameManager.Instance.p1.Heal(recover);
            //healthp1.Heal(recover);
            renderer.enabled = !renderer.enabled;
            audio.Play();
            Destroy(gameObject, 2f);
        }
        if (other.gameObject.name == "player2" && GameManager.Instance.p2.health < 100)
        {
